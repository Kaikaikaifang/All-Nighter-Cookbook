...
%% 参数设置
% TODO 输入波形选择
dataSourceType = 1; % 波形类型 0表示正弦波,1表示三音正弦波,2表示方波
...
figure(4);
subplot(2, 1, 1);
plotTimeDomain(params.t, y, '原始信号', '时间/s', '电压/v');
subplot(2, 1, 2);
plotTimeDomain(tout, yout, '滤波后的时域波形', '时间/s', '电压/v');

% 绘制输入和输出信号的频域波形
figure(5);
subplot(2, 1, 1);
plotFrequencyDomain(y, params.fs, params.N, '原始信号的频谱', '频率/hz', '电压/v');
subplot(2, 1, 2);
[yout_fft, ~] = plotFrequencyDomain(yout, params.fs, params.N, '滤波后的频谱', '频率/hz', '电压/v');

% 绘制输入和输出信号的功率谱
figure(6);
subplot(2, 1, 1);
y_psd = y_fft .* conj(y_fft);
plotPowerSpectrum(f, y_psd, '输入信号功率谱', '频率/Hz', 'W/Hz');
subplot(2, 1, 2);
yout_psd = yout_fft .* conj(yout_fft);
plotPowerSpectrum(f, yout_psd, '输出信号功率谱', '频率/Hz', 'W/Hz');
...
Rx_max = max(Rx);
plotAutoCorrelation(maxlags / params.fs, Rx / Rx_max, '输入信号自相关函数', '时间/s', 'R(t)');
subplot(2, 1, 2);
[Rx1, maxlags1] = xcorr(yout, 'unbiased'); % 信号的自相关
Rx_max = max(Rx1);
plotAutoCorrelation(maxlags1 / params.fs, Rx1 / Rx_max, '输出信号自相关函数', '时间/s', 'R(t)');
